...
const CcontentFetch = () => {
        ...
        const getBlogs = async () => {
            try {
                ...
                if (sortedData.length > 0) {
                    setLastUpdated(sortedData[0].updatedAt); // Assuming sorted by updatedAt descending
                }
            } catch (error) {
                console.error("Error fetching data:", error);
                setLoading(false);
    ...
    const renderPageNumbers = () => {
        // Generate pagination with first two pages, ellipsis, current page, and last two pages
        const pageNumbers = [];
        if (totalPages <= 5) {
            for (let i = 1; i <= totalPages; i++) {
                pageNumbers.push(
            ...
            // Logic for ellipsis and current page display
            // (code omitted for brevity, same as in your current implementation)
        }
        return pageNumbers;
    };
    ...
    return (
        <>
            <div className="flex flex-col sm:flex-row lg:flex-row justify-between text-2xl font-semibold dark:bg-slate-900 dark:text-white text-gray-700 mb-5 pt-5 pb-5 pl-5 lg:pl-20 dark:from-slate-800">
                <div className="flex flex-col space-y-4">
                    <span>Basic Interview Questions – C Programming.</span>
                ...
                <div className="mt-4 sm:mt-0 lg:mt-4 sm:mr-20 lg:mr-20">
                    <a href="path-to-your-file.pdf" download>
                        <button className="bg-white text-sm dark:bg-slate-900 text-red-500 border border-red-500 px-5 py-3 rounded-md">
                            Download PDF
                        </button>
                    </a>
                </div>
                ...
                <div className="mx-auto text-left">
                    <div className="w-full p-3 sm:p-5 md:p-8 lg:p-10 max-w-[950px] mx-auto border-t md:shadow-sm md:shadow-black/30 rounded-xl bg-[rgb(255,255,255)] dark:bg-slate-800 dark:border-none mt-3">
                        {loading ? (
                            ...
                            paginatedData.map((item) => <GetContent key={item.id} item={item} />)
                        )}
                    </div>
                </div>
