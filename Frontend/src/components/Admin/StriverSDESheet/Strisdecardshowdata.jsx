...
import Stricardupdatebtncomp from './Stricardupdatebtncomp';
...
const NoteList = () => {
    ...
    useEffect(() => {
        const fetchNotes = async () => {
            try {
                const response = await axios.get('http://localhost:4001/sheet');
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`http://localhost:4001/sheet/${id}`);
    ...
    const openDeleteModal = (id) => {
        setDeleteNoteId(id); // Set the note id to delete
    };
                    ...
                    <table className="table-auto border-collapse border border-gray-300 w-full">
                        <thead>
                            <tr className="bg-gray-100">
                                ...
                                <th className="border border-gray-300 px-4 py-2">Problems</th>
                        ...
                        <tbody>
                            {notes.map(note => (
                                <tr key={note._id} className="text-center">
                                    ...
                                    <td className="border border-gray-300 px-4 py-2">{note.Problems}</td>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
                    <div className="bg-white p-8 rounded shadow-lg flex flex-col">
                        <Stricardupdatebtncomp note={updatingNote} onUpdateSuccess={handleUpdateSuccess} onClose={closeModal} />
                    </div>
                </div>
            )}
