...
const NoteList = () => {
    ...
    useEffect(() => {
        const fetchNotes = async () => {
            try {
                const response = await axios.get('http://localhost:4001/sheetproblem');
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`http://localhost:4001/sheetproblem/${id}`);
            ...
            {isModalOpen && (
                <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
                    <div className="bg-white p-8 rounded shadow-lg flex flex-col">
                        <Striproblemupdatebtncomp note={updatingNote} onUpdateSuccess={handleUpdateSuccess} onClose={closeModal} />
                    </div>
                </div>
            )}
