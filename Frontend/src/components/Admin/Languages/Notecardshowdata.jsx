...
import UpdateNoteForm from './Noteupdatebuttoncom';
...
const NoteList = () => {
    ...
    useEffect(() => {
        const fetchNotes = async () => {
            try {
                const response = await axios.get('http://localhost:4001/note');
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`http://localhost:4001/note/${id}`);
                        ...
                        <tbody>
                            {notes.map(note => (
                                <tr key={note._id} className="text-center">
                                    ...
                                    <td className="border border-gray-300 px-4 py-2">{note.price}</td>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
                    <div className="bg-white p-8 rounded shadow-lg flex flex-col">
                        <UpdateNoteForm note={updatingNote} onUpdateSuccess={handleUpdateSuccess} onClose={closeModal} />
                    </div>
                </div>
            )}
